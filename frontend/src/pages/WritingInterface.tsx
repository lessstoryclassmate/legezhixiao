...
import { Layout, Button, Space, Spin, message, Tooltip } from 'antd'
import { 
  SaveOutlined, 
  RobotOutlined, 
  HistoryOutlined,
...
const WritingInterface: React.FC = () => {
  ...
  const handleFileUploadSuccess = (files: any) => {
    console.log('[WritingInterface] 文件上传成功', files)
    
    if (Array.isArray(files)) {
      // 多文件处理 - 合并内容
      const combinedContent = files.map((file: any) => 
        `\n\n=== 导入文件：${file.fileName} ===\n${file.content || ''}`
      ).join('\n\n')
      
      if (editorRef.current) {
        const currentContent = editorRef.current.getValue()
        const newContent = currentContent + combinedContent
        editorRef.current.setValue(newContent)
      ...
      if (editorRef.current) {
        const currentContent = editorRef.current.getValue()
        const newContent = currentContent + importedContent
        editorRef.current.setValue(newContent)
        setContent(newContent)
      }
      
      message.success(`成功导入文件：${file.fileName}`)
    }
  ...
  const handleFileUploadError = (error: any) => {
    console.error('[WritingInterface] 文件上传失败', error)
