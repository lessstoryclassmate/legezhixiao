...
export class ProjectService {
  ...
  constructor() {
    // 确保PouchDB已初始化
    pouchDBService.initialize().catch(console.error);
  }
  ...
  async createProject(projectData: Partial<NovelProject>): Promise<NovelProject> {
    ...
    // 保存到数据库
    await pouchDBService.create('project', project);
  ...
  async getAllProjects(): Promise<NovelProject[]> {
    const docs = await pouchDBService.query('project');
    return docs.map(doc => doc.data as NovelProject)
  ...
  async getProject(projectId: string): Promise<NovelProject | null> {
    const doc = await pouchDBService.read(projectId);
    if (!doc || doc.type !== 'project') {
      return null;
    }
    return doc.data as NovelProject;
  }
  ...
  async updateProject(projectId: string, updates: Partial<NovelProject>): Promise<NovelProject | null> {
    ...
    const doc = await pouchDBService.update(projectId, updatedData);
    if (!doc) return null;

    const project = doc.data as NovelProject;
  ...
  async deleteProject(projectId: string): Promise<boolean> {
    const success = await pouchDBService.delete(projectId);
