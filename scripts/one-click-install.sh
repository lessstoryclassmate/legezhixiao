...
get_user_config() {
    ...
    if [ -z "$JWT_SECRET_KEY" ]; then
        read -p "JWT Secret Key (可为空，将自动生成): " JWT_SECRET_KEY
        if [ -z "$JWT_SECRET_KEY" ]; then
            JWT_SECRET_KEY=$(openssl rand -base64 32 2>/dev/null || echo "auto-generated-jwt-secret-$(date +%s)")
        fi
    fi
