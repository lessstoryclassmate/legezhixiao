...
if [ -n "$MONGODB_HOST" ]; then
  ...
  if timeout 10 bash -c "</dev/tcp/$MONGODB_HOST/${MONGODB_PORT:-27017}" 2>/dev/null; then
    ...
    echo "建议检查："
    echo "- MongoDB 服务状态: ssh $MONGODB_HOST 'sudo systemctl status mongod'"
...
if [ -n "$REDIS_HOST" ]; then
  ...
  if timeout 10 bash -c "</dev/tcp/$REDIS_HOST/${REDIS_PORT:-6379}" 2>/dev/null; then
    ...
    echo "建议检查："
    echo "- Redis 服务状态: ssh $REDIS_HOST 'sudo systemctl status redis'"
...
echo "   - 检查服务运行状态: systemctl status <service>"
echo ""
echo "2. MongoDB (172.16.32.2:27017) 问题："
echo "   - 服务状态: sudo systemctl status mongod"
...
echo "   - 服务状态: sudo systemctl status redis"
...
echo "   - 服务状态: sudo systemctl status mysql"
